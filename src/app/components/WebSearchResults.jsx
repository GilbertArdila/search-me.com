...
const WebSearchResults = ({results}) => {
  return (
    <div className='w-full mx-auto px-3 pb-40 sm:pl-[5%] md:pl-[14%] lg:pl-4'>
        ...
        {results.items?.map((result) => (
            <div className='mb-8 max-w-xl' key={result.title}>
                <div className='group flex flex-col'>
                    <Link className='text-sm truncate' href={result.link}>{result.formattedUrl}</Link>
                    <Link className='group-hover:underline decoration-blue-500 text-xl truncate font-medium text-blue-800' href={result.link}>{result.title}</Link>
